...
function Profile() {
  ...
  useEffect(() => {
    if (localStorage.getItem("token")) {
      ...
      GetAsync("/userImages/read_one.php?id=" + profileDetails.userId) // Getting the profile image from the database
        .then((res) => {
      ...
      >
        ...
        <div className="details">
          <span>
            ...
            <h3>User Name: {profileDetails.userName}</h3>
            <h3>Email :{profileDetails.email}</h3>
            <h3>Phone Number: {profileDetails.phoneNumber}</h3>{" "}
            <h3>Gender: {profileDetails.gender}</h3>
          </span>
        </div>
        <ChangePassword />
      </Card>
